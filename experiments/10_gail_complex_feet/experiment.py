...
from musculoco_il.util.rewards import OutOfBoundsActionCost, ActionSmoothnessReward, \
...
def build_agent(mdp, expert_data, use_cuda, discrim_obs_mask, train_D_n_th_epoch=3,
    ...
    if checkpoint_path is not None:
        ...
        if agent._sw is not None:
            # Apply same hack as in GAIL_TRPO constructor to allow deepcopy
            setattr(agent._sw, '__deepcopy__', lambda _: None)
        
        # Update env_reward_frac if provided
        if hasattr(agent, 'env_reward_frac'):
            agent.env_reward_frac = env_reward_frac
        
        # Optionally update learning rates for fine-tuning
        if hasattr(agent, '_critic'):
            for param_group in agent._critic._optimizer.param_groups:
                param_group['lr'] = lrc
        if hasattr(agent, '_discriminator'):
            for param_group in agent._discriminator._optimizer.param_groups:
                param_group['lr'] = lrD
...
def experiment(n_epochs: int = 500,
    ...
    elif reward_type == 'action_smoothness':
        print(f'Using custom Reward: {reward_type}')
        reward_type_loco = 'custom'
        reward_callback = ActionSmoothnessReward(reward_scale=env_reward_scale)
        reward_params = dict(reward_callback=reward_callback)
    elif reward_type == 'action_magnitude':
        print(f'Using custom Reward: {reward_type}')
        reward_type_loco = 'custom'
        reward_callback = ActionMagnitudePenalty(reward_scale=env_reward_scale)
        reward_params = dict(reward_callback=reward_callback)
    elif reward_type == 'smooth_and_magnitude':
        print(f'Using custom Reward: {reward_type}')
        reward_type_loco = 'custom'
        # Use L2 penalty (power=2) to allow sustained low-level activity
        reward_callback = CombinedReward([
    ...
    # Add passive stiffness to unactuated foot joints to prevent "floppy feet"
    if not use_box_feet:
        print("Adding passive stiffness to foot joints (subtalar, mtp)...")
        ...
        for j_name in foot_joints:
            try:
                ...
                # Set stiffness to a biomechanically reasonable range (e.g., 20-30 Nm/rad)
                # This simulates the passive elastic properties of ligaments/plantar fascia.
