...
class GRFStructureReward(object):
    """Simple GRF-structure reward to encourage physiologically plausible walking.

    Terms:
    - support consistency: discourage unrealistically low total vertical support force
    - impact smoothing: penalize abrupt vertical GRF changes
    - bilateral balance: discourage extreme left-right vertical imbalance
    """
    ...
    def __init__(self,
        ...
        self._model = None
        self._data = None
        self._prev_fz_l = None
        self._prev_fz_r = None
    ...
    def __call__(self, state, action, next_state):
        ...
        fz_l = float(np.abs(grf_l[2]))
        fz_r = float(np.abs(grf_r[2]))
        fz_total = fz_l + fz_r

        bw = self.body_mass_kg * 9.81

        # 1) Support consistency: avoid unrealistic low support in stance evolution
        support_target_min = 0.45 * bw
        support_pen = max(0.0, support_target_min - fz_total) / max(1e-6, bw)

        # 2) Impact smoothing: reduce sharp GRF spikes from one step to the next
        if self._prev_fz_l is None:
            impact_pen = 0.0
        else:
            df_l = abs(fz_l - self._prev_fz_l) / max(1e-6, bw)
            df_r = abs(fz_r - self._prev_fz_r) / max(1e-6, bw)
            impact_pen = 0.5 * (df_l + df_r)

        # 3) Bilateral balance: avoid one-leg overload for prolonged periods
        balance_pen = abs(fz_l - fz_r) / max(1e-6, bw)

        self._prev_fz_l = fz_l
        self._prev_fz_r = fz_r

        total_pen = (
            self.support_weight * support_pen
            + self.impact_weight * impact_pen
            + self.balance_weight * balance_pen
        )
        return -self.reward_scale * total_pen
...
def experiment(n_epochs: int = 500,
               ...
               grf_support_weight: float = 1.0,
               grf_impact_weight: float = 0.05,
               grf_balance_weight: float = 0.1,
               seed: int = 0):
    np.random.seed(seed)
    ...
    elif reward_type == 'grf_structure':
        print(f'Using custom Reward: {reward_type}')
        reward_type_loco = 'custom'
        reward_callback = GRFStructureReward(
            body_mass_kg=body_mass_kg,
            reward_scale=env_reward_scale,
            support_weight=grf_support_weight,
            impact_weight=grf_impact_weight,
            balance_weight=grf_balance_weight,
        )
        reward_params = dict(reward_callback=reward_callback)
