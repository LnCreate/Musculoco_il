...
from musculoco_il.algorithms.AMP import AMP_TRPO
...
from musculoco_il.policy.attention_synergy_torch_policy import AttentionSynergyPolicy
...
def build_agent(mdp, expert_data, use_cuda, discrim_obs_mask, train_D_n_th_epoch=3,
                ...
                amp_gp_weight=10.0, amp_logit_reg=0.0, amp_replay_size=200000, amp_replay_keep_prob=1.0,
                env_reward_frac=0.0, standardize_obs=True, learn_latent_layer=False,
                use_attention_synergy=True, n_synergies=16,
    ...
    latent_dim = int(n_synergies) if use_attention_synergy else 256
    ...
    if use_attention_synergy:
        policy_params.update(dict(
            synergy_attn_dim=synergy_attn_dim,
            synergy_temperature=synergy_temperature,
        ))
    ...
    alg_params = dict(train_D_n_th_epoch=train_D_n_th_epoch,
                      ...
                      discriminator_entcoeff=d_entr_coef,
                      amp_gp_weight=amp_gp_weight,
                      amp_logit_reg=amp_logit_reg,
                      amp_replay_size=amp_replay_size,
                      amp_replay_keep_prob=amp_replay_keep_prob,
    ...
    policy_class = AttentionSynergyPolicy if use_attention_synergy else LatentExplorationPolicy
    print(f'POLICY_CLASS: {policy_class.__name__} | n_synergies={latent_dim}')

    agent = AMP_TRPO(mdp_info=mdp_info, policy_class=policy_class, policy_params=policy_params, sw=sw,
...
def experiment(n_epochs: int = 500,
               ...
               amp_gp_weight: float = 10.0,
                amp_logit_reg: float = 0.0,
               amp_replay_size: int = 200000,
               amp_replay_keep_prob: float = 1.0,
               env_freq: int = 1000,
               ctrl_freq: int = 100,
               ...
               use_attention_synergy: bool = True,
               n_synergies: int = 16,
               synergy_attn_dim: int = 64,
    ...
    agent = build_agent(mdp=mdp, expert_data=expert_data, use_cuda=use_cuda,
                        ...
                        use_attention_synergy=use_attention_synergy,
